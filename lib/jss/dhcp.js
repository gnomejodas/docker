...
//                            contLista = data.split("\n");
                            ...
                            for (i =0; i< redes.length ; i++){
                                ...
                                arrayredes.push(redes[i][0]);
                                                                                                
                                tabla.innerHTML += "<tr><td>" + redes[i][0] + "</td> <td>"+ redes[i][1] + " </td></tr>";
                            }
...
function pedirConfiguracion(){
            ...
            $.post( "index.php?site=dhcp", 
                            { pedirConfiguracion: "1"},
                            function( data ) {
                            var config = JSON.parse(data);
//                            alert(config[1]);
                            var tabla = document.getElementById("dhcpdata");
                            tabla.innerHTML= "";
//                            contLista = data.split("\n");
                            
                            for (i =0; i< config[0].length ; i++){
                                                                                                
                                tabla.innerHTML += "<tr><td>" + config[0][i] +" </td></tr>";
                            }
                            tabla.innerHTML += config[1];
...
//               Convertimos todos los campos del formulario a Binario
    ...
    var mascReducida = formulario.m1.value;
...
//                Aquí comprobamos si la primera dirección es anterior a la segunda
    else if (parseInt(bintodec(miporciondehost(ipstart,reducida))) >= parseInt(bintodec(miporciondehost(ipend,reducida)))){

        alert("la primera dirección de cesión debe ser menor a la segunda");
        alert(bintodec(miporciondehost(ipstart,reducida)));
        alert(bintodec(miporciondehost(ipend,reducida)));
...
//                alert(dnsDHCP);
         ...
         $.post( "./index.php?site=dhcp", 
                ...
                fin: finDHCP, gateway: gateDHCP, dns: dnsDHCP, mask: mascReducida},
                function( data ) {
                   //alert(data);
                 alert(data);
